...
In the Czech timber industry, logs are typically cut according to predefined patterns without regard for internal structure. This approach leads to suboptimal lumber yields, because flaws such as knots, cracks, and rot remain concealed inside the log and are not identified until after processing. Computed tomography (CT) offers a way to reveal these internal defects, allowing sawmills to produce higher-quality planks and increase economic returns. However, no publicly available CT dataset of logs with voxel-level defect annotations that conforms to Czech technical standards exists.

A dataset of 40 CT scanned oak logs provided by the Czech University of Life Sciences Prague (CZU) is used to address this gap. A substantial portion of the work is devoted to the annotation of defects within this dataset, which forms the basis for subsequent analysis and modeling. The annotated data are then used to develop and evaluate methods for detecting and classifying defects within the 3D structure of the logs.

The primary goal of this thesis is to create a CT dataset of oak logs with defect categories defined according to industry standards. Additionally, the thesis aims to develop and evaluate algorithms capable of reliably detecting and classifying defects within the 3D volume of a log. The results are intended to serve as the foundation for a system that optimizes cutting strategies and improves the economic efficiency of wood processing in the Czech Republic.
...
Another approach presented by~\cite{khazem2023deep} analyzes CT scans of wood logs using a ResNet-34 regression model for pith detection, along with a U-Net for contour extraction and knot segmentation. The study was conducted on a diverse dataset comprising 682 trees of 23 species. For knot segmentation, the U-Net achieved a Dice score of 0.698 when trained on 2,504 annotated images with data augmentation and demonstrated the ability to generalize to previously unseen species. However, this approach utilized each CT slice separately and therefore did not take advantage of the full 3D spatial context. The authors highlighted that convolutional networks struggle with representing intricate knot configurations and proposed the use of self-attention blocks. This motivates the adoption of a transformer architecture, such as SwinUNETR~\cite{hatamizadeh2022swinunetr}, which operates directly on 3D volumes using the self-attention to model complex spatial relationships more effectively. 
...
U-Net~\cite{ronneberger2015unet} was the most popular base architecture, chosen by 64\% of the
participating teams. Furthermore, the best performing methods shared the use of Dice loss and data augmentation. The
winning method, nnU-Net~\cite{isensee2021nnunet},
demonstrated that preprocessing, architecture
topology, and a training strategy optimized to the properties of each dataset were more impactful than the novel architectural designs.

A recent contribution relevant to thin elongated structures is the Skeleton Recall Loss introduced by~\cite{kirchhoff2024skeletonrecall}. The loss precomputes a tubed skeleton of the ground truth on the CPU and rewards predictions that cover the medial axis, which preserves connectivity in classes that are only a few voxels wide but extend over long distances, while avoiding the GPU memory overhead of differentiable skeletonization methods. The method was integrated into nnU-Net and contributed to the first place finishes on the TopCoW circle of the Willis vessel challenge~\cite{topcow2023}, the ToothFairy inferior alveolar canal challenge~\cite{toothfairy2023}, and the recent RSNA Intracranial Aneurysm Detection challenge~\cite{rsna2025aneurysm}.
...
The experimental dataset provided by the Faculty of Forestry and Wood Science of the Czech University of Life Sciences Prague (CZU) consists of CT scans created by a Siemens Somatom Scope Power CT scanner. The system
...
($I_0$). By recording these values from thousands of angles, the system solves for the linear attenuation coefficient ($\mu$) in every voxel (volumetric pixel).
...
By definition, distilled water is calibrated to 0~HU and air to $-1000$~HU. In the context of oak logs, this scale provides a clear separation between defects of low density such as cracks or rot, which approach $-1000$~HU, and denser features such as healthy wood, ranging from approximately $-600$ to $+500$~HU, and knots, which usually range from $+300$~HU to $+600$~HU depending on species and moisture~\cite{freyburger2009measuring}.
...
unsigned 16-bit integers. That said, because Hounsfield Units can be negative
...
\noindent where \text{RescaleSlope} and \text{RescaleIntercept} are parameters specific to individual scanners.
...
A key ability of the Swin Transformer is its efficient self-attention mechanism. Rather than computing self-attention over the entire volume, which is computationally expensive, the feature map is partitioned into local windows. Self-attention is then computed independently within each window, and in subsequent layers these windows are shifted, enabling information exchange across neighboring regions. By alternating between these two operations, the model is able to capture both local structure and broader spatial context while maintaining computational efficiency.

The encoder structure aligns well with the U-Net decoder design. Features extracted at different resolutions are passed through skip connections to the corresponding decoder stages, improving spatial localization and supporting more precise segmentation. The implementation provided by MONAI v1.4~\cite{cardoso2022monai} was used in this work.
...
In general, MedNeXt provides a strong alternative to classical U-Net variants and transformer-based models, combining convolutional efficiency and inductive bias with improved receptive field modeling and achieving strong performance on medical image segmentation tasks. In this work, the implementation provided by MONAI v1.4~\cite{cardoso2022monai} was used.
...
Due to the time requirements of the annotation process, a subset of six logs was selected from the forty available. Four of these logs were chosen for their high defect density to ensure that all target classes were sufficiently represented. The remaining two logs were comparatively healthy. They were included to prevent the models from associating natural wood grain with defects and to expose them to logs that are defect-sparse. In total, this selected subset yielded 6~230 slices for annotation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{images/sample_slice_full.png}
    \caption{Example of an unprocessed CT slice from the CZU dataset, shown at the original $512 \times 512$ in-plane resolution. The log occupies the central region, surrounded by air; internal structures such as knots and rot are visible by eye.}
...
\State Apply CLAHE to $I$ to obtain $I'$
...
With the selected and preprocessed subset of six logs, the annotation phase was executed using the CVAT platform introduced in Section~\ref{sec:annotation_tool_selection}.
...
series. The background and bark masks created by
Algorithms~\ref{alg:logmask} and~\ref{alg:bark} were imported as
pre-existing labels. As a result, each slice arrived with the two outermost
classes already segmented, leaving only the internal defects to be drawn.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{FITthesis-LaTeX-master/images/original.png}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{FITthesis-LaTeX-master/images/annotated.png}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{FITthesis-LaTeX-master/images/og.png}
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{FITthesis-LaTeX-master/images/annotated2.png}
...
Since each defect spans multiple consecutive CT slices, propagating annotations through the volume was a significant part of the workflow. CVAT's tracking functionality\footnote{\url{https://docs.cvat.ai/docs/annotation/manual-annotation/modes/track-mode-basics/}}, based on the Segment Anything Model 3~\cite{sam3}, was used to assist in this process. A mask defined on the slice $n$ was propagated to the slice $n+1$, where it was accepted, rejected, or manually refined.

This approach worked well for structures that gradually change between slices, such as knots and cracks. In contrast, it was less effective for more shallow defects, including rot and insect damage, for which manual labeling proved more effective.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
...
\chapter{Analysis}
The annotated oak logs exhibit complex spatial characteristics and severe class imbalances. These structural challenges dictate the subsequent experimental design, specifically driving the data split strategy, the selection of three segmentation architectures (nnU-Net, MedNeXt, and SwinUNETR), and the adoption of a specialized loss function for internal wood defect segmentation.
...
Holding out a complete log rather than partitioning every volume reflects the goal of evaluating generalization across trees rather than across slices of the same tree. Slices within a single log are highly correlated, since neighboring slices differ by only one millimeter of wood, so a slicewise split would leak information across subsets. The voxel coverage of the test log \textit{dub3} is reported in Figure~\ref{fig:test_coverage} and follows the same overall distribution as the training data described in Section~\ref{sec:dataset_characteristics}, which means that the test log is representative of the dataset as a whole.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{images/test_coverage.png}
    \caption{Class coverage in the test volume \textit{dub3}. The distribution follows the overall dataset characteristics reported in Section~\ref{sec:dataset_characteristics}.}
    \label{fig:test_coverage}
\end{figure}

To ensure a diverse validation set and prevent the model from overfitting to the specific morphology of a single tree, the five remaining logs were partitioned along the longitudinal axis into smaller sections. Each section contained roughly 300 CT slices, representing approximately 20~cm of wood. By dividing the logs into smaller sub-volumes rather than relying on full volumes, the validation set could be constructed from multiple distinct trees, capturing a much wider variance of internal structural configurations.
...
A cross-validation scheme was considered, but a single fixed split was used instead, prioritizing time and compute on hyperparameter and architecture tuning over repeated validation runs. Drawing the validation sections from different logs at distinct longitudinal positions also partly compensates for the variability of a single fixed split.
...
\begin{equation}
\mathcal{L}_{\text{Dice}} = 1 - \frac{1}{|C_{\text{fg}}|} \sum_{c \in C_{\text{fg}}} \frac{2 \sum_i p_{i,c} \, g_{i,c} + \epsilon}{\sum_i p_{i,c} + \sum_i g_{i,c} + \epsilon}
\label{eq:dice}
\end{equation}

where $p_{i,c}$ and $g_{i,c}$ denote the predicted probability and ground truth value of class $c$ at voxel $i$, $C_{\text{fg}}$ is the set of foreground classes, and $\epsilon$ is a smoothing constant. The background is excluded so that its size does not dominate the loss.

Weighted cross-entropy reweights the standard cross-entropy using the inverse of the class frequency to ensure errors in rare defect classes are heavily penalized:

\begin{equation}
\mathcal{L}_{\text{CE}} = -\sum_i \sum_c w_c \, g_{i,c} \log p_{i,c}
\label{eq:ce}
\end{equation}

where $g_{i,c}$ and $p_{i,c}$ are the ground truth and predicted probability of class $c$ at voxel $i$, and $w_c$ is the weight of class $c$, set inversely proportional to the frequency of $c$ in the training set.
...
The nnU-Net baseline retained the framework's default training configuration, using SGD with Nesterov momentum optimization and a polynomial learning rate schedule starting at $10^{-2}$. The patch size of $[224, 128, 224]$ voxels was set after splitting the original CT volumes into smaller sub-volumes for cross-validation, and the batch size remained at the framework's default of 2.

\subsection{Augmentation}

nnU-Net employs an automatically configured augmentation pipeline, as summarized in Table~\ref{tab:nnunet_augmentation}. This includes random rotations, scaling, axis flips, Gaussian noise, blurring, brightness and contrast adjustments, gamma corrections, and elastic deformations. The augmentation parameters are derived directly from the dataset fingerprint, which includes image size and voxel spacing, so that all transformations are appropriately scaled to the characteristics of the dataset.

\begin{table}[htbp]
\centering
\caption{The data augmentation pipeline used by the nnU-Net framework. Transforms are applied dynamically during training via the \texttt{batchgenerators} library. (Note: Parameter ranges are framework defaults and scale relative to the dataset fingerprint).}
\label{tab:nnunet_augmentation}
\small
\begin{tabularx}{\textwidth}{llX}
    \toprule
    \textbf{Stage} & \textbf{Transform} & \textbf{Parameters} \\
    \midrule
    Spatial       & \texttt{SpatialTransform}            & scaling [0.7, 1.4], rotation adjusted for dataset anisotropy \\
                  & \texttt{MirrorTransform}             & $p=0.5$, valid spatial axes \\
    \midrule
    Photometric   & \texttt{GaussianNoiseTransform}      & $p=0.1$ \\
                  & \texttt{GaussianBlurTransform}       & $p=0.2$ \\
                  & \texttt{BrightnessMultiplicativeTransform} & $p=0.15$ \\
                  & \texttt{ContrastAugmentationTransform} & $p=0.15$ \\
                  & \texttt{SimulateLowResolutionTransform} & $p=0.25$ \\
                  & \texttt{GammaTransform}              & $p=0.1$--$0.3$, global and per-channel \\
    \bottomrule
\end{tabularx}
\end{table}

\subsection{Training Dynamics}
The training and validation losses, along with the pseudo Dice heuristic reported by the nnU-Net framework, were monitored to assess learning and convergence. The pseudo Dice is a batchwise approximation of Dice that is computed during training rather than from full inference on the validation volumes. It is therefore not directly comparable to the Dice foreground validation reported for SwinUNETR and MedNeXt.

Figure~\ref{fig:nnunet_training_curve} shows a rapid reduction in initial loss over 140 epochs. The training loss (blue) decreases smoothly, while the validation loss (red) is noticeably noisier and sits above the training loss for most of the run. This reflects the small validation set, where each validation pass evaluates only four sections drawn from four different logs, so small shifts in which patches are sampled produce visible epoch-to-epoch variation. The exponential moving average of the pseudo Dice (green) smooths out this noise and converges to a plateau just below 0.82. After training, the framework recomputes the foreground mean Dice from full inference on the validation volumes, and this exact metric is the value reported along with SwinUNETR and MedNeXt in the comparison Table~\ref{tab:all_experiments}.

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{images/nnunet_loss.png}
\caption{Training and validation loss}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{images/nnunent_dice.png}
\caption{Validation pseudo Dice}
\end{subfigure}
\caption{Training dynamics of the nnU-Net model. The plotted metric in (b) is the framework's pseudo Dice, which differs from the validation foreground mean Dice used for the other models.}
\label{fig:nnunet_training_curve}
\end{figure}
\subsection{Pretrained Weights}

A custom trainer \texttt{nnUNetTrainerLungPretrained} was implemented to initialize encoder weights from a Lung CT segmentation checkpoint (Task006\_Lung). Lung CT was selected as the source domain because lung structures share some characteristics with the oak log volumes, including internal low density cavities and regions of varying density that produce comparable Hounsfield Unit contrasts. The segmentation head was retrained from scratch to match the output class count. This initialization did not improve validation Dice or convergence compared to training from scratch.


\section{SwinUNETR}
\label{sec:swinunetr_config}
...
As summarized in Table~\ref{tab:all_experiments}, several combinations of learning rates and patch dimensions were evaluated. The optimal configuration utilized a pretrained encoder, a learning rate of $10^{-4}$, and 3D patches of size $224 \times 180 \times 224$ voxels. The axial dimensions of $224 \times 224$ match the square in-plane shape of the CT scans, which measure $512 \times 512$ voxels. The longitudinal size of 180 voxels was strictly bounded by GPU memory limits; the available A100 with 40\,GB of VRAM prevented a larger extent, and any increase above 180 caused out-of-memory errors. For this final model, the batch size was set to 4 with gradient accumulation over 4 iterations. Gradient norms were reduced to 1.0 and a dropout rate of 0.2 was applied to reduce overfitting.
...
\begin{tabularx}{\textwidth}{llX}
    ...
    Spatial       & \texttt{RandFlipd}            & p = 0.5, axial axes only \\
                  ...
                  & \texttt{RandAffined}          & p = 0.25, rotation $\pm 30^\circ$ around longitudinal axis, translation 10 vox, scale $\pm 15$\,\% \\
    \midrule
    ...
    \midrule
    Sampling      & \texttt{RandCropByLabelClassesd} & Background 1.0, healthy wood 1.0, bark 2.0, knot 4.0, crack 5.0, rot 6.0, insect damage 8.0 \\
    \bottomrule
\end{tabularx}
\end{table}
...
Figure~\ref{fig:swinunetr_training_curve} shows the loss and validation foreground mean Dice over 113 epochs. The training loss drops sharply in the first 20 epochs, then converges more gradually. The validation Dice rises rapidly until epoch 30 and increases more slowly afterwards, reaching 0.721 at epoch 113.

Training was stopped when the cluster time limit expired. The validation curve had not fully plateaued, which suggests that further training could have improved the score. Early stopping based on a fixed plateau criterion was not applied, since the goal was to compare three architectures under a comparable compute budget rather than to find the absolute best run for SwinUNETR.
...
\caption{Validation foreground mean Dice}
\end{subfigure}
...
This initialization improved validation Dice and convergence compared to training from scratch. Yet, the improvement was limited. Although there are structural similarities at the CT level, the domain gap between medical anatomy and wood restricted the extent of meaningful feature transfer.

\section{MedNeXt}

MedNeXt was trained using the AdamW optimizer~\cite{loshchilov2019adamw} with the same learning rate, weight decay, and warmup schedule as SwinUNETR~\ref{sec:swinunetr_config}, namely an initial learning rate of $10^{-3}$, a weight decay of $10^{-4}$, and a 20 epoch linear warmup increasing the learning rate tenfold from $10^{-4}$ to $10^{-3}$. The patch size of $224 \times 180 \times 224$ voxels, the batch size of 4 with gradient accumulation over 4 iterations, the gradient clipping at 1.0, and the dropout rate of 0.2 were also retained from the SwinUNETR configuration.
...
Figure~\ref{fig:mednext_training_curve} shows the loss and validation foreground mean Dice over 168 epochs. The training loss falls below 3.0 within 25 epochs, then decreases more slowly, reaching 1.88 on the training set while the validation loss stabilizes around 2.0. The validation Dice peaks at 0.761 at epoch 161.

The training to validation Dice gap is smaller for MedNeXt than for SwinUNETR. Larger gaps in transformer training on small corpora are documented in the literature~\cite{antonelli2022msd}, and the MedNeXt result is consistent with this pattern, although a single training run on one dataset cannot establish the cause.

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[height=4.5cm]{images/mednext_loss.png}
\caption{Training and validation loss}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[height=4.5cm]{images/mednext_dice.png}
\caption{Validation foreground mean Dice}
\end{subfigure}
...
\ForAll{defect classes $d \in \{\text{knot, rot, bark, crack, insect}\}$}
  ...
  \ForAll{connected components $H$ in $F \setminus \text{mask}(d)$ with original label $\in \{\text{BG, HW}\}$ and $|H| \leq 50\,000$}
...
All three models were evaluated on the test log \textit{dub3} using mean Dice coefficient and IoU as primary metrics. The foreground mean excludes the background class and reflects the performance of the defect categories.

\section{Overall Performance}

\begin{table}[H]
\centering
\caption{Summary of training experiments. Test foreground Dice and IoU are reported only for the final submitted model in each architecture. Post* is the test foreground mean Dice after rule-based postprocessing following Algorithm~\ref{alg:postprocess}, and IoU* is the corresponding foreground mean IoU on the postprocessed predictions.}
\label{tab:all_experiments}
\begin{adjustbox}{max width=\textwidth}
\begin{tabular}{llrrrr}
\toprule
\textbf{Configuration} & \textbf{Loss} & \textbf{Best val.\ Dice} & \textbf{Test fg.\ Dice} & \textbf{Post*} & \textbf{IoU*} \\
\midrule
\multicolumn{6}{l}{\textit{nnU-Net v2 (ResEnc-L planner)}} \\
\midrule
2d, $512 \times 512$                                & Dice + CE             & 0.704 & --- & --- & --- \\
2d, $512 \times 512$                                & Dice + CE + Skeleton  & 0.730 & --- & --- & --- \\
3d\_fullres, $224 \times 128 \times 224$            & Dice + CE             & 0.793 & --- & --- & --- \\
3d\_fullres, $224 \times 128 \times 224$            & Dice + CE + Skeleton  & 0.802 & --- & --- & --- \\
3d\_fullres, $256 \times 160 \times 256$            & Dice + CE + Skeleton  & 0.805 & --- & --- & --- \\
3d\_cascade, $224 \times 128 \times 224$            & Dice + CE             & 0.799 & --- & --- & --- \\
3d\_cascade, $224 \times 128 \times 224$            & Dice + CE + Skeleton  & 0.807 & --- & --- & --- \\
3d\_cascade, $256 \times 160 \times 256$            & Dice + CE + Skeleton  & \textbf{0.813} & \textbf{0.775} & \textbf{0.833} & \textbf{0.724} \\
\midrule
\multicolumn{6}{l}{\textit{SwinUNETR (custom framework)}} \\
\midrule
Not pretrained, LR $5 \times 10^{-4}$, $128 \times 256 \times 128$ & Dice + CE             & 0.590 & --- & --- & --- \\
Pretrained encoder, LR $10^{-4}$, $128 \times 384 \times 128$      & Dice + CE             & 0.644 & --- & --- & --- \\
Pretrained encoder, LR $5 \times 10^{-4}$, $128 \times 256 \times 128$ & Dice + CE         & 0.683 & --- & --- & --- \\
Pretrained encoder, LR $10^{-4}$, $224 \times 180 \times 224$      & Dice + CE             & 0.701 & --- & --- & --- \\
Pretrained encoder, LR $10^{-4}$, $224 \times 180 \times 224$      & Dice + CE + Skeleton  & \textbf{0.721} & \textbf{0.702} & \textbf{0.756} & \textbf{0.613} \\
\midrule
\multicolumn{6}{l}{\textit{MedNeXt (custom framework)}} \\
\midrule
LR $10^{-3}$, $128 \times 256 \times 128$                          & Dice + CE             & 0.710 & --- & --- & --- \\
LR $10^{-3}$, $128 \times 256 \times 128$                          & Dice + CE + Skeleton  & 0.723 & --- & --- & --- \\
LR $10^{-3}$, $128 \times 384 \times 128$                          & Dice + CE + Skeleton  & 0.735 & --- & --- & --- \\
LR $10^{-3}$, $224 \times 180 \times 224$                          & Dice + CE + Skeleton  & \textbf{0.761} & \textbf{0.744} & \textbf{0.783} & \textbf{0.656} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

Table~\ref{tab:all_experiments} summarizes the training experiments and the test performance of the final submitted model for each architecture. The 3D cascade nnU-Net achieves the highest scores, followed by MedNeXt and SwinUNETR. The gap between nnU-Net and MedNeXt is 0.050 mean Dice on the postprocessed test predictions, and the gap between nnU-Net and SwinUNETR is 0.077.
...
nnU-Net leads on every class, but several margins are narrow. Healthy wood and bark remain the highest scoring classes for all three models, reflecting their large extent and clearer textural separation. The lowest scoring class differs between architectures. For nnU-Net and MedNeXt, insect damage is the weakest class, consistent with its very low voxel coverage. For SwinUNETR, rot and insect damage tie at 0.683, and knot also drops to 0.706, which suggests that SwinUNETR struggles with classes whose boundaries depend on long-range context as much as it struggles with sparsely distributed classes. Across all three architectures, the smallest and most sparsely distributed classes consistently produce the lowest Dice values.
...
\begin{table}[H]
    \centering
    \caption{Row-normalized confusion matrix for nnU-Net on the test log \textit{dub3}. Values are percentages of each true class. Bold diagonal entries show correctly classified voxels. BG stands for background and HW stands for healthy wood.}
    \label{tab:cmnnunet}
    \begin{adjustbox}{max width=\textwidth}
...
Background and healthy wood achieve high recall at 98.0\,\% and 98.1\,\%, and bark reaches 98.4\,\%. Insect damage has the lowest recall at 73.4\,\%, with 19.1\,\% of insect voxels predicted as healthy wood and a further 7.3\,\% predicted as bark, consistent with its very small voxel coverage and sparse distribution near the log surface. Knot reaches 74.3\,\%, with 25.5\,\% of knot voxels predicted as healthy wood, reflecting the gradual HU transition between dense knot tissue and the surrounding wood. Rot scores 89.5\,\% with 10.4\,\% confused with knot, consistent with the spatial proximity of the two classes shown in the distance matrix in Figure~\ref{fig:distance_matrix}. Crack reaches 93.9\,\% with 6.1\,\% leaking to healthy wood. A smaller pattern is the boundary confusion between bark and the surrounding tissue, where 1.0\,\% of background and 1.1\,\% of healthy wood voxels are predicted as bark, and 0.7\,\% of bark voxels are predicted as healthy wood. These errors concentrate at the log surface, where contrast between bark and the layers immediately on either side is low.
...
\begin{figure}[p]
  ...
  \caption{In (h), a knot above the crack is missing and the other knot boundaries are also inaccurate. In (j), the lower elongated knot is missing its upper portion. In (l), the rot boundaries are slightly inaccurate and a central knot is missing.}
\end{figure}
...
\begin{figure}[p]
    ...
    % Row 1 -- Longitudinal Example 1 (e.g., continuous crack)
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/long_pred1gt.png}
        \caption{Ground truth: Healthy wood, Bark, Knot, Rot}
        \label{fig:long_row1_gt}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/long_pred1.png}
        \caption{Accurate: Knot, Rot; Inaccurate: Healthy wood, Bark}
        \label{fig:long_row1_pred}
    \end{subfigure}

    \vskip\baselineskip

    % Row 2 -- Longitudinal Example 2
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        % Notice the added height and keepaspectratio parameters!
        \includegraphics[width=\textwidth, height=0.25\textheight, keepaspectratio]{images/long_pred2gt.png}
        \caption{Ground truth: Healthy wood, Bark, Knot, Rot}
        \label{fig:long_row2_gt}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/long_pred2.png}
        \caption{Accurate: Knot, Rot; Inaccurate: Healthy wood, Bark}
        \label{fig:long_row2_pred}
    \end{subfigure}

    \vskip\baselineskip

    % Row 3 -- Longitudinal Example 3 (e.g., failure case / z-axis disconnection)
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/long_pred4gt.png}
        \caption{Ground truth: Healthy wood, Bark, Crack, Knot, Rot}
        \label{fig:long_row3_gt}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.47\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/long_pred4.png}
        \caption{Accurate: Bark, Crack, Rot; Inaccurate: Healthy wood, Knot}
        \label{fig:long_row3_pred}
    \end{subfigure}
    
  \caption{Longitudinal views extracted from the test volume \textit{dub3}. In (b) and (d), the internal structures of knots and rot are accurate along the longitudinal axis, while portions of the bark are misclassified. In (f), the model successfully tracks the vertical continuity of the crack and rot, but fails to accurately delineate the knot from the surrounding healthy wood.}
  \label{fig:qualitative_longitudinal}
\end{figure}


\chapter{Discussion}
The three architectures are compared against each other, followed by an examination of how annotation quality and inherent class ambiguity shaped the observed error patterns. Finally, the dataset limitations that affect how broadly the results generalize are discussed.

\section{Architecture Comparison}

Table~\ref{tab:all_experiments} places nnU-Net ahead of both MedNeXt and SwinUNETR on every aggregate metric, and Table~\ref{tab:perclass_results} confirms that the lead is consistent across classes after postprocessing. The 3D cascade configuration likely contributed the most to this result. The low resolution stage processes the entire volume at once and captures global context such as the position of a defect within the log and the overall shape of larger structures like rot regions or knot clusters. The full-resolution stage then refines these predictions with localized detail, recovering thin cracks and sharp boundaries. This combination is well suited to a dataset where defects span a wide range of scales, from a few voxels of insect damage to centimeter scale rot.

SwinUNETR finished last with a foreground mean Dice of 0.756. The shifted window attention that defines the architecture is data hungry, and the substantial gap between training and validation Dice indicates that the encoder did not see enough volumes to learn stable global dependencies. Transformers reward scale and 16 training sections do not deliver it. This is consistent with the Medical Segmentation Decathlon~\cite{antonelli2022msd}, where transformer variants tended to lag behind well configured CNNs on smaller tasks. The architecture is not a poor fit for volumetric wood defect segmentation in principle, it is a poor fit for the size of the corpus available here.

MedNeXt landed between the two, trailing nnU-Net by 0.050 Dice and leading SwinUNETR by 0.027. The large depthwise kernels that define this network were intended to approximate the broad receptive fields of attention while keeping the convolutional inductive bias intact. On this dataset they appear slightly oversized, since the wider receptive field carries more parameters than the smaller convolutions in nnU-Net and the small training corpus does not fully utilize that extra capacity. The per-class breakdown in Table~\ref{tab:perclass_results} is also more divided than the aggregate suggests. MedNeXt beats SwinUNETR on classes that are locally distinguishable by texture and density, namely healthy wood, rot, and bark, but loses on classes whose boundaries depend more on long-range context such as knot, crack, and insect damage, where transformer attention apparently helps more than enlarged convolutions.
...
The confusion matrix in Table~\ref{tab:cmnnunet} shows the same pattern in the prediction of the model. 25.5\,\% of knot voxels are predicted as healthy wood, 10.4\,\% of rot voxels are predicted as knot, and the bark boundary leaks in three directions at once (1.0\,\% bark to background, 0.7\,\% bark to healthy wood, and 1.1\,\% healthy wood to bark). These are not arbitrary errors. They sit on the same boundaries that made annotation difficult.
...
This thesis addressed the detection and classification of defects in oak logs scanned by computed tomography. All tasks defined in the assignment were completed.
...
The defect categories were defined according to ČSN~EN~1310~\cite{csnen1310} and ČSN~EN~1311~\cite{csnen1311} (Section~\ref{sec:label_selection}). Four defect classes were selected, namely cracks, knots, rot, and insect damage. Bark was added as a structural class because separating it from the log interior is necessary to recover the usable wood volume.

A complete annotation methodology was designed and executed (Chapter~\ref{chap:dataset_annotation}, including the CVAT workflow in Section~\ref{sec:cvat_workflow}). Six logs were selected from the forty available, four of them with high defect density and two comparatively healthy. Background and bark masks were created automatically using Otsu thresholding~\cite{otsu1979threshold} and SLIC superpixel clustering~\cite{achanta2012slic}, which provided initial templates for the CVAT annotation pipeline. Defects inside the logs were manually annotated with the help of SAM~3~\cite{sam3} based propagation between slices. The final corpus contains 6\,230 annotated CT slices.

The detection and classification algorithm builds on three 3D semantic segmentation architectures, namely nnU-Net~\cite{isensee2021nnunet}, MedNeXt~\cite{roy2023mednext}, and SwinUNETR~\cite{hatamizadeh2022swinunetr}. A custom loss combining Dice~\cite{milletari2016vnet}, weighted cross-entropy, and skeleton recall~\cite{kirchhoff2024skeletonrecall} was used to handle class imbalance and the topology of thin elongated structures such as cracks. Four rule-based postprocessing steps were derived from the class distance analysis to correct systematic misclassifications.

Results on the test log \textit{dub3} are reported in Chapter~\ref{chap:results}. The 3D cascade nnU-Net achieved the best foreground mean Dice of 0.833, followed by MedNeXt at 0.783 and SwinUNETR at 0.756. Postprocessing improved every model, with the largest gain on cracks. The row-normalized confusion matrix in Table~\ref{tab:cmnnunet} and the per-class Dice values in Table~\ref{tab:perclass_results} describe the remaining error modes, most of which concentrate on the gradient transitions between knot, rot, and the surrounding wood.
...
Importantly, the segmentation results achieved a high Dice score (0.981) on the healthy wood class. For estimating an optimal cutting strategy, accurately localizing the continuous regions of healthy wood is the most important factor, as this directly dictates the maximum volume of quality planks that can be extracted. The dataset, the annotation pipeline, and the trained models together form the foundation for a system that informs sawmill cutting strategies. Future work includes scanning a larger and more diverse set of logs, extending the annotation effort beyond oak, and integrating the segmentation output with cutting plan optimization to translate voxel-level predictions into measurable yield gains.